...
// Read index.html at startup so Express can serve it on any environment
const INDEX_HTML_PATH = path.join(__dirname, "..", "index.html");
let indexHtmlContent = "";
try {
  indexHtmlContent = fs.readFileSync(INDEX_HTML_PATH, "utf8");
} catch (e) {
  console.warn("⚠️  Could not read index.html:", e.message);
}
...
// ─── Serve index.html for all non-API routes (works on both local and Vercel) ───
app.get("*", (req, res) => {
  if (indexHtmlContent) {
